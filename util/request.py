...
def get_yesterday_arxiv_papers(category: str = "cs.CV", max_results: int = 100):
    ...
    for i in range(0, len(entries), 2): 
        ...
        title = title_tag.text.strip().replace('Title:', '').strip() if title_tag else "No title available"
        ...
        comments = comments_tag.text.strip() if comments_tag else "No comments available"
